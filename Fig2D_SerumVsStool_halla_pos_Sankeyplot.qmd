...
hallagram <- hallagram0 |> 
  ...
  filter(Y_features %in% data2$Stool_metabolites) |> filter(`q-values` < 0.05) |> filter(association > 0)
...
filtered_long1 <- 
  add_hmdb_classification(filtered_long |> rename(Feature = node), n_class = 13) |> 
  rename(node = Feature)
