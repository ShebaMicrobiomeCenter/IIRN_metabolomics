...
Stool_longcor1 <- pnID_function(Stool_couples,Stool_map) |> mutate(Type = "Stool")
s16_longcor1 <- pnID_function(s16_couples,s16_map) |> mutate(Type = "16S")
mgx_ecs_longcor1 <- pnID_function(mgx_ecs_couples,mgx_map) |> 
  mutate(Type = "mgx_ecs")
mgx_paths_longcor1 <- pnID_function(mgx_paths_couples,mgx_map) |> 
  mutate(Type = "mgx_paths")
mgx_species_longcor1 <- pnID_function(mgx_species_couples,mgx_map) |> 
  mutate(Type = "mgx_species")
...
intra_perm_results1 <- 
  perm_test(x = Serum_longcor1 |> filter(intrapn == 'Serum_Within') |> select(coral),
            y = Stool_longcor1 |> filter(intrapn == 'Feces_Within') |> select(coral))

intra_perm_results2 <- 
  perm_test(x = Stool_longcor1 |> filter(intrapn == 'Feces_Within') |> select(coral),
            y = s16_longcor1 |> filter(intrapn == '16s_Within') |> select(coral))

inter_perm_results1 <- 
  perm_test(x = Serum_longcor1 |> filter(intrapn == 'Serum_Across')|> select(coral),
            y = Stool_longcor1 |> filter(intrapn == 'Feces_Across')|> select(coral))
...
all_longcor1 <- all_longcor1 |> 
  mutate(Type = str_replace(Type, "Serum|Stool", paste0(Type,"\nMetabolomics"))) |> 
  mutate(Type = str_replace(Type, "mgx_","MGX\n"))

all_longcor1$Type <- 
  factor(all_longcor1$Type, levels = c("Serum\nMetabolomics","Stool\nMetabolomics",
                                       "16S","MGX\nspecies","MGX\necs","MGX\npaths"))
...
all_longcor1 |> ggplot(aes(x=type_n, y=coral, fill = intrapn), show.legend = FALSE) + 
